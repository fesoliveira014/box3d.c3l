...
- `box3d.c3i` — types + raw `extern fn ... @cname(...)` declarations, grouped by area rather than by declaration kind. Everything else is idiomatic wrappers, one file per area: `box3d.c3` (base hooks), `box3d_world.c3`, `box3d_body.c3`, `box3d_shape.c3`, `box3d_event.c3` (the world's event arrays as slices), `box3d_query.c3` (queries and casts), `box3d_mesh.c3` and `box3d_compound.c3` (complex geometry), `box3d_joint.c3` (the shared joint surface) and one `box3d_joint_<type>.c3` per joint type. Two files are not wrappers: `box3d_check.c3` — `faultdef`s + the per-type `.checked()` macros (`WorldId.checked()`, `BodyId.checked()`, …) that map a null identifier to a named fault — and `box3d_layout.c3`, the compile-time `$assert` layout pins generated by `scripts/build-box3d.sh`; never edit that one by hand. **Every `.c3` and `.c3i` at the package root declares the same `module b3;`** — the files keep the library's name, the module takes the C symbol prefix. Do not enumerate them here: the list went stale at four consecutive slices, and a new area adds a file.
