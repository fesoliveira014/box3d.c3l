...
- `box3d.c3i` — types + raw `extern fn ... @cname(...)` declarations. `box3d.c3` — idiomatic wrappers for the base hooks. `box3d_world.c3` — idiomatic wrappers for the world lifecycle. `box3d_body.c3` — idiomatic wrappers for the body lifecycle. `box3d_check.c3` — `faultdef`s + a per-type `.checked()` macro (`WorldId.checked()`, `BodyId.checked()`, …) that maps a null identifier to a named fault. `box3d_layout.c3` — the compile-time `$assert` layout pins, generated by `scripts/build-box3d.sh`; never edit it by hand. All six declare the same `module b3;` — the files keep the library's name, the module takes the C symbol prefix.
