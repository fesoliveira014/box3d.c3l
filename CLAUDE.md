...
- `manifest.json` — `provides` the `box3d` module; `linklib-dir` is `linked-libs`. Consumers depend on `box3d` (the `provides` name), not on the directory name.
- `box3d.c3i` — types + raw `extern fn ... @cname(...)` declarations. `box3d.c3` — idiomatic wrappers. `box3d_check.c3` — `faultdef`s + the `check()` that maps C failure signals to faults. All three declare the same `module box3d;`.
...
`manifest.json` sets no `sources`, and it does not need to: every `.c3` and `.c3i` at the package root is compiled into the library. Verified — a `fn` defined in `box3d.c3` is callable from `test/`.
...
- **The C prefix never appears on the C3 side.** `box3d::create_world`, not `b3CreateWorld`. Functions `snake_case`, types `PascalCase`, constants and enum values `SCREAMING_SNAKE_CASE` — `b3` stripped from all of them. No `@builtin`; it defeats the namespace.
